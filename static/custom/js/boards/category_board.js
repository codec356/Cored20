...
function ul_li_listener() {
    let town_block = document.querySelector('#town-filter');
    let town = town_block.querySelectorAll('li');
    [].forEach.call(town, function (el) {
        el.addEventListener('click', function () {
            [].forEach.call(town, function (element) {
                element.classList.remove('selected');
            });
            el.classList.add('selected');
            l_town_id = el.dataset.townId;
            reloadRegions();
            getAjaxOffers();
        })
    });
...
function reloadRegions() {
    $.ajax({
        ...
        "success": function (response) {
            console.log(response);
            let regions_block = document.querySelector('#region-filter ul');
            regions_block.innerHTML = `<li data-region-id="0" class="selected">모두</li>`;
            for (let rec in response) {
                if (response.hasOwnProperty(rec)) {
                    regions_block.insertAdjacentHTML("beforeend", `<li data-region-id="${response[rec]['pk']}">${response[rec]['fields']['name']}</li>`)
                }
            }
            let regions = regions_block.querySelectorAll('li');
            [].forEach.call(regions, function (el) {
